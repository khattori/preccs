...
let errmsg = function
    ERR_UNDEF_VAR(s)       -> "undefined variable \"" ^ Symbol.name(s) ^ "\""
  | ERR_UNDEF_LABEL(s)     -> "undefined label \"" ^ Symbol.name(s) ^ "\""
...
let error s = print_string "Error: "; print_string s; print_newline()
let errorAt fi e = printInfo fi; error(errmsg e); raise (Exit 1)

let warning s = print_string "Warning: "; print_string s; print_newline()
let warningAt fi w = printInfo fi; warning(warmsg w)
