...
proc CreateGate(n:int, ret:<Gate>) =
   var in:<unit>; var out:<unit>; var bell:<unit>;
   var gate = { in=in; out=out; bell=bell; n=n };
   ret!gate
proc Main() = 
    var ret:<Gate>;
    CreateGate(9,ret); ret?reindeer_gate;
    CreateGate(3,ret); ret?elf_gate;
...
proc Worker(id:int,gate:Gate,kind:string,job:string) =
  var ret:<unit>; RandomSleep(1000,ret); ret?x;
  gate.in!();
  gate.out?x;
  C{ printf("%s: %d %s\n", STRPTR($kind$), TOCINT($id$), STRPTR($job$)); C};
  Worker(id, gate, kind, job)

proc RandomSleep(n:int, ret:<unit>) =
   var rand:int;
